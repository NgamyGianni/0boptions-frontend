...
const ActualGame = ({userInfos, idCurrentGame}) => {
	...
	async function getGameInfos(idGame){
			...
			setGameInfos({
				...
				PriceEnd: currentPrice.toFixed(3),
	...
	useEffect(() => {
		if(idCurrentGame !== 0){
			...
			/*if(gameInfos !== "")	setTimeout(() => {setCounter(counter+1);}, 15000)
			else	setTimeout(() => {setCounter(counter+1);}, 4000)*/
		}
	},[idCurrentGame]);
	return (
				<Stats>
