...
const Betting = ({userInfos, idCurrentGame}) => {
	...
	async function betUp(){
		...
		if(Boolean(inputAmount.current.value.match("^[0-9]*[.,]?[0-9]*$"))){
			let betAmount = String(parseFloat(inputAmount.current.value.replace(",", ".")) * 1000000000000000000)
		 	await contract.methods.joinUp().send({from: userInfos.account, value: betAmount});
		 }
	}
	...
	async function betDown(){
		...
		if(Boolean(inputAmount.current.value.match("^[0-9]*[.,]?[0-9]*$"))){
			let betAmount = String(parseFloat(inputAmount.current.value.replace(",", ".")) * 1000000000000000000)
		 	await contract.methods.joinDown().send({from: userInfos.account, value: betAmount});
		 }
	}
